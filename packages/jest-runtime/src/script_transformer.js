...
export default class ScriptTransformer {
  ...
  transformSource(filepath: Path, content: string, instrument: boolean) {
    ...
    const willTransform = transform && shouldTransform(filename, this._config);

    // That means that the transform has a custom instrumentation
    // logic and will handle it based on `config.collectCoverage` option
    const transformerInstruments =
      transform && willTransform && transform.canInstrument;
    ...
    const mapCoverage = instrument && !transformerInstruments;
    ...
    if (!transformerInstruments && instrument) {
